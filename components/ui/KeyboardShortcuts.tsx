  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-6 left-6 z-40 rounded-full w-14 h-14 shadow-lg bg-gray-800 hover:bg-gray-700 flex items-center justify-center text-gray-400 hover:text-white transition-colors"
            ...
            <div key={index} className="flex items-center justify-between p-3 bg-gray-800 rounded-lg">
              ...
              <div className="flex gap-1">
                {shortcut.keys.map((key, keyIndex) => (
                  <kbd
                    key={keyIndex}
                    className="px-2 py-1 text-xs font-semibold text-gray-200 bg-gray-700 border border-gray-600 rounded shadow-sm"
                  >
                    {key}
                  </kbd>
                ))}
